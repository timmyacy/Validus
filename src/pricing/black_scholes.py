...
class BlackScholesFX:
    ...
    def calculate_d1_d2(option: FXOption):
        ...
        d1 = (
            np.log(spot / strike)
            + (domestic_rate - foreign_rate + volatility**2 / 2) * time_to_maturity
    ...
    def price(option: FXOption, d1: float, d2: float, multiplier: float):
        ...
        if option.option_type == OptionType.CALL:
            unit_pv = option.spot_price * fr * stats.norm.cdf(
                d1
            ) - option.strike * dr * stats.norm.cdf(d2)
        else:
            unit_pv = option.strike * dr * stats.norm.cdf(
                -d2
            ) - option.spot_price * fr * stats.norm.cdf(-d1)
    ...
    def calculate_vega(option: FXOption, d1: float, multiplier: float):
        ...
        vega = (
            fr
            * option.spot_price
            * np.sqrt(option.time_to_maturity)
            * stats.norm.pdf(d1)
            * 0.01
        )
